/* *******************************
Calf.c version 1.0 by D. Martel
Juin 2018

Programme de production de calendrier, version française
...
#include "calf.h"   /* Date manipulation structures header */
 ...
 */
  ...
  if (argc > 0)    /**** verify each argument to be a numeral */
     for (i=1; i < argc; i++)
      if ( !isnumeral(argv[i]) ) {
        printf("\nErreur ! Un ou plusieurs paramètres ne sont pas numériques. L'usage est : année [mois1 [mois2]]\n");
        return 1;
      }
  ...
  switch ( argc  ) {
    case 1 :  /* no argument error */
      printf("\nErreur! Aucun paramètre fourni ! L'usage est : année [mois1 [mois2]]\n");
      return 1;
    ...
    case 3 :  /* year and one month only */
      ...
      sscanf(argv[2], "%i", &m1);
      if ( (m1 < 1) || (m1 > 12) ) {
        printf("\n%s%s", argv[2], " : valeur incorrecte pour le mois : doit-être entre 1 et 12.\n");
        return 1;
      }
      k = 1;
      break;
    ...
    case 4 :  /* we've got a year and two months to form an interval */
      ...
      if ( (m1 < 1) || (m1 > 12) || (m2 < 1) || (m2 > 12) || (m1 == m2) ) {
        //printf("\n%s: Valeur incorrecte pour le mois : doit-être entre 1 et 12.\n", argv[0]);
        printf("\n%s, %s : au moins l'un des mois est incorrect ! Doit-être entre 1 et 12.\n", argv[2], argv[3]);
        return 1;
      }
      ...
      if (m1 < m2) /* stay in same year */
        ...
        k = m2+12-m1+1;
        if (y+1 > MAXYEAR) {
          printf("\nL'année deviendra plus grande que %i pour l'intervalle choisie !\n", MAXYEAR);
          return 1;
        }
      }
   } /* end switch */
 ...
 /* Validate the year */
    if ( y < MINYEAR || y > MAXYEAR) {
      printf("\n%s : l'année n'est pas correcte ! Doit être : 1 < année <= %i\n", argv[1], MAXYEAR);
      return 1;
    }
...
/* This function prints 1 to 3 calendars from consecutive months.
   ...
   Other members of struct st_cal cal used to control execution.
*/
