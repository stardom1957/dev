/* *******************************
Cala.c version 1.0 by D. Martel
Juin 2018

Calendar print program, English version
...
int main(int argc, char *argv[])
 ...
 struct st_cal cal[12];     // we print 12 months max
 ...
 */
  ...
  if (argc > 0)    /**** verify each argument to be a numeral */
     for (i=1; i < argc; i++)
      if ( !isnumeral(argv[i]) ) {
        printf("\nError! One or more parameters are not numbers. Usage is: year [month1 [month2]]\n");
        return 1;
      }
  ...
  switch ( argc  ) {
    case 1 :  /* no argument error */
      ...
      printf("\nError! No parameter provided! Usage is: year [month1 [month2]]\n");
      return 1;
    ...
    case 3 :  /* year and one month only */
      ...
      if ( (m1 < 1) || (m1 > 12) ) {
        ...
        printf("\n%s%s", argv[2], " : Invalid value for month: must be between 1 and 12.\n");
        return 1;
      }
      k = 1;
      break;
    ...
    case 4 :  /* we've got a year and two months to form an interval */
      ...
      if ( (m1 < 1) || (m1 > 12) || (m2 < 1) || (m2 > 12) || (m1 == m2) ) {
        ...
        printf("\n%s, %s : at least one month is invalid! Must be between 1 and 12.\n", argv[2], argv[3]);
        return 1;
      }
      ...
      else { /**** step up to next year, like 2018 11 1 means: November 2018 to January 2019 */
        k = m2+12-m1+1;
        if (y+1 > MAXYEAR) {
          //printf("%s: ", "Year will become invalid in chosen interval.\n", argv[0]);
          printf("\nYear will become greater than %i for choosen interval!\n", MAXYEAR);
          return 1;
        }
      }
   } /* end switch */
 ...
 /* Validate the year */
    if ( y < MINYEAR || y > MAXYEAR) {
      ...
      printf("\n%s : year is not valid! Must be: 1 < year <= %i\n", argv[1], MAXYEAR);
      return 1;
    }
